...
# Find the path of libmkl_core.so
LIBMKL_PATH=$(find ~/anaconda3/envs/webshop/lib -name 'libmkl_core.so*' -type f)
# Check if libmkl_core.so was found
if [ -z "$LIBMKL_PATH" ]; then
    echo "Error: libmkl_core.so not found!"
    exit 1
fi
# Extract directory from the path
LIBMKL_DIR=$(dirname "$LIBMKL_PATH")
# Export the LD_LIBRARY_PATH
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$LIBMKL_DIR
