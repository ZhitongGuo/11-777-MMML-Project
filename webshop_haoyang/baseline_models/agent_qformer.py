...
from models.custom_qformer import QFormerConfigForWebshop, QFormerModelForWebshop, QFormerFrozenModelForWebshop

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

State = namedtuple('State', ('obs', 'goal', 'click', 'estimate', 'obs_str', 'goal_str', 'raw_image'))
...
class Agent:
    def __init__(self, args, image_processor, freeze_qformer=True):
        print("Freeze QFormer: {}".format(freeze_qformer))
        # tokenizer
        ...
        # network
        config = QFormerConfigForWebshop(image=True)
        if freeze_qformer:
            self.network = QFormerFrozenModelForWebshop(config, token_embed_size=len(self.tokenizer))
        else:
            self.network = QFormerModelForWebshop(config, token_embed_size=len(self.tokenizer))
        if args.qformer_path != '':
            print("Loading QFormer from {}".format(args.qformer_path))
            self.network.load_state_dict(torch.load(args.qformer_path, map_location=torch.device('cpu')), strict=False)
    ...
    def build_state(self, ob, info):
        ...
        if raw_image is None:
            image_tensor = torch.ones((3, IMAGE_SIZE, IMAGE_SIZE), dtype=torch.float32)
        else:
            image_tensor = self.image_processor(raw_image, return_tensors="pt")['pixel_values'] # [0]
        if len(image_tensor.shape) == 3:
            image_tensor = torch.unsqueeze(image_tensor, dim=0)
        return State(obs_ids, goal_ids, click, estimate, obs_str, goal_str, image_tensor)
