...
class WebEnv:
    ...
    def __init__(self, args, split, server=None, id=None, feat_conv=None, cache=None, url2asin=None):
        ...
        # print(f"Start, RSS: {memory_info.rss / (1024 * 1024):.2f} MB")
        
        if args.num is None:
            if split == 'test':
                self.goal_idxs = range(500)
            elif split == 'eval':
                self.goal_idxs = range(500, 1000)
            elif split == 'train':
                self.goal_idxs = range(1000, len(self.env.server.goals) // 2)
        else:
            self.goal_idxs = range(len(self.env.server.goals))
        ...
        # memory_info = psutil.Process().memory_info()
        # print(f"Before, RSS: {memory_info.rss / (1024 * 1024):.2f} MB")

        assert cache is not None

        self.cache = cache
        if self.cache["asin2name"] is None:
            self.cache["asin2name"] = {k.lower(): v['Title'].lower(
            ) for k, v in self.env.server.product_item_dict.items()}
        self.asin2name = self.cache["asin2name"]

        # print("ASIN2name loaded")

        # memory_info = psutil.Process().memory_info()
        # print(f"After, RSS: {memory_info.rss / (1024 * 1024):.2f} MB")

        if self.cache["name2asin"] is None:
            self.cache["name2asin"] = {v: k for k, v in self.asin2name.items()}
        self.name2asin = self.cache["name2asin"]
        ...
        # memory_info = psutil.Process().memory_info()
        # print(f"End, RSS: {memory_info.rss / (1024 * 1024):.2f} MB")
    ...
    def step(self, action):
        ...
        info.update({'valid': self.get_valid_actions(), 'goal': self.env.instruction_text,
                     ...
                     'prev_ob': self.prev_ob, 'desc': desc, 'feat': feat, 'raw_image': None
                     })
        
        if self.get_image:
            image_feat = self.env.get_image()
            raw_image = self.env.get_raw_image()
            info['image_feat'] = image_feat
            info['raw_image'] = raw_image
    ...
    def reset(self, idx=None):
        ...
        info.update({'valid': self.get_valid_actions(), 'goal': self.env.instruction_text,
                     ...
                     'prev_ob': self.prev_ob, 'desc': '', 'feat': '', 'raw_image': None
                     })
        self.steps = 0
