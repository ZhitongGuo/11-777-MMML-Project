...
import wandb
wandb.init(mode="dryrun") ## prevent from loading data to the server
...
if model_choice == "blip2":
    # AutoProcessor, AutoTokenizer, Blip2Model
...
try:
    ...
    tokenizer.tokenizer.add_tokens(['[button]', '[button_]', '[clicked button]',
                        '[clicked button_]'], special_tokens=True)
    print(len(tokenizer.tokenizer))
...
def get_dataset(split, mem=False):
    ...
    if model_choice == "blip2":
        ...
        print(state_encodings['input_ids'].shape, state_encodings['attention_mask'].shape)
    elif model_choice == "blip":
        state_encodings = tokenizer(
            None, states, padding='max_length', max_length=512, truncation=True, return_tensors='pt')
        print(state_encodings['input_ids'].shape, state_encodings['attention_mask'].shape)
        action_encodings = tokenizer(
            None, actions, padding='max_length', max_length=128, truncation=True, return_tensors='pt')
    dataset = {
...
def main():
    ...
    config = BlipConfigForWebshop(
        image=args.image, pretrain_bert=args.pretrain)
    if model_choice == "blip":
        model = BlipModelForWebshop(config, token_embed_size=30526, blip1 = True) #len(tokenizer.tokenizer)
        print("blip model loaded")
    elif model_choice == "blip2":
        model = BlipModelForWebshop(config, token_embed_size=len(tokenizer), blip1 = False)
        print("blip model loaded")
    else:
